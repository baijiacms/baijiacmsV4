...
if ($_W["isajax"]) {
    ...
    $goods    = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,o.title as optiontitle,g.isverify,g.storeids from " . tablename("eshop_order_goods") . " og " . " left join " . tablename("eshop_goods") . " g on g.id=og.goodsid " . " left join " . tablename("eshop_goods_option") . " o on o.id=og.optionid " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array(
        ":uniacid" => $uniacid,
        ":orderid" => $orderid
    ));
    $openids  = array();
