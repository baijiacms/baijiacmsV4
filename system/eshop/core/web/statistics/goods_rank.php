...
$sql = 'SELECT g.id,g.title,g.thumb,' . '(select ifnull(sum(og.price),0) from  ' . tablename('eshop_order_goods') . ' og left join ' . tablename('eshop_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as money," . '(select ifnull(sum(og.total),0) from  ' . tablename('eshop_order_goods') . ' og left join ' . tablename('eshop_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition}) as count  " . 'from ' . tablename('eshop_goods') . ' g  ' . "where 1 {$condition1}  order by {$orderby} desc ";
