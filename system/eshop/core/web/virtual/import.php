...
if ($operation == 'temp') {
    ...
    foreach ($datas as $d) {
        $olddata = pdo_fetch('SELECT * FROM ' . tablename('eshop_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid ', array(
            ':pvalue' => $d['pvalue'],
            ':typeid' => $_GPC['typeid'],
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($olddata)) {
            pdo_insert('eshop_virtual_data', $d);
            pdo_update('eshop_virtual_type', 'alldata=alldata+1', array(
                'id' => $item['id']
            ));
        } else {
            if (empty($olddata['openid'])) {
                pdo_update('eshop_virtual_data', $d, array(
                    'id' => $olddata['id']
                ));
            } else {
                $noinsert .= $d['pvalue'] . ',';
    ...
    if (!empty($noinsert)) {
        ...
        message('导入成功！' . $tip, $this->createWebUrl('virtual/data', array(
            'typeid' => $_GPC['typeid']
        )));
    }
}