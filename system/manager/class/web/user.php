	  ...
	  $operation = !empty($_GP['op']) ? $_GP['op'] : 'listuser';
	   if ($operation == 'listuser') {
	 	 ...
	 	 foreach ( $list as $id => $item) {
					 	 	...
					 	 	{
			             	 	$list[$id]['store']=getStoreBeid($item['beid']);
       				}           	
                }
	  	  ...
	  	  if ($operation == 'edituser') {
	  	  		...
	  	  		$account = mysqld_select('SELECT * FROM '.table('user')." WHERE  id=:id" , array(':id'=> $_GP['id']));
						...
						if(empty($account['id']))
						{
							...
							$data= array('beid'=>$_GP['store'],'is_admin'=> intval( $_GP['is_admin']),'username'=> $_GP['username'],'password'=> md5($_GP['newpassword']),'createtime'=>time());
					...
					$data= array('username'=> $_GP['username'],'beid'=>$_GP['store'],'is_admin'=> intval( $_GP['is_admin']));
							 mysqld_update('user', $data,array('id'=>$account['id']));
							 
							 
									message('编辑用户成功！',web_url('user'),'succes');	
