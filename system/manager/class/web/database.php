...
function dump_export($continue = array()) {
 ...
 if (checksubmit("submit")) {
		...
		if(!empty($continue)) {
			...
			message('正在导出数据, 请不要关闭浏览器, 当前第 1 卷.', create_url('site', array('act' => 'manager','do' => 'database','op'=>'display','ctu'=>$postctu)),'success');
		} else {
				message('数据已经备份完成', create_url('site', array('act' => 'manager','do' => 'database','op'=>'display')),'success');
		}
	}
	if($_GP['ctu']) {
		...
		if(!empty($continue)) {
			$postctu=base64_encode(json_encode($continue));
			message('正在导出数据, 请不要关闭浏览器, 当前第 ' . $ctu['series'] . ' 卷.', create_url('site', array('act' => 'manager','do' => 'database','op'=>'display','ctu'=>$postctu)),'success');
		} else {
			message('数据已经备份完成', create_url('site', array('act' => 'manager','do' => 'database','op'=>'display')),'success');
		}
	}
