...
if (!class_exists('DesignerModel')) {
    ...
    {public function getSet()
	    	...
	    	return globalSetting('designer');	
	    	}
        public function getPage($type = 1)
        {
            global $_W, $_GPC;
            $page = pdo_fetch("SELECT * FROM " . tablename('eshop_designer') . " WHERE uniacid= :uniacid and pagetype=:type and setdefault=:default", array(
                ':uniacid' => $_W['uniacid'],
                ':type' => $type,
                ':default' => '1'
            ));
        ...
        public function getData($page)
        {
            ...
            if (count($arr) > 0) {
                $goodinfos = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,sales,unit FROM " . tablename('eshop_goods') . " WHERE id in ( " . implode(',', $arr) . ") and uniacid= :uniacid ", array(
                    ':uniacid' => $_W['uniacid']
                ), 'id');
            ...
            $shopset=array();
            $shopset['shop']=  globalSetting('shop');  
             $shopset['share']=  globalSetting('share');  
        ...
        public function getGuide($system, $pageinfo)
        {
            ...
            if (!empty($_GPC['preview'])) {
                ...
                $guide['openid2']  = m('user')->getOpenid(true);
            ...
            if ($guide['followed'] != '1') {
                ...
                if (!empty($_GPC['shareid'])) {
                    $guide['member1'] = pdo_fetch("SELECT id,nickname,openid,avatar FROM " . tablename('eshop_member') . " WHERE openid=:mid and uniacid= :uniacid limit 1 ", array(
                        ':uniacid' => $_W['uniacid'],
                        ':mid' => $_GPC['shareid']
                    ));
                    $guide['member2'] = pdo_fetch("SELECT id,nickname,openid FROM " . tablename('eshop_member') . " WHERE openid=:openid and uniacid= :uniacid limit 1 ", array(
                        ':uniacid' => $_W['uniacid'],
        ...
        {
            global $_W;
            return pdo_fetchcolumn('select id from ' . tablename('eshop_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
        ...
        {
            global $_W;
            return pdo_fetch('select * from ' . tablename('eshop_designer_menu') . ' where isdefault=1 and uniacid=:uniacid limit 1', array(
                ':uniacid' => $_W['uniacid']
            ));
        }
