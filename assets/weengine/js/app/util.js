(function(window) {
	...
	util.iconBrowser = function(callback){
		...
		var modalobj = util.dialog('请选择图标',[window.public_utility_selectIcon+'&callback=selectIconComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
	...
	util.emojiBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog('请选择表情',[window.public_utility_selectEmoji+'&callback=selectEmojiComplete'],footer,{containerName:'icon-container'});
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-dialog').css({'width':'70%'});
		modalobj.find('.modal-body').css({'height':'70%','overflow-y':'scroll'});
		modalobj.modal('show');
	...
	util.linkBrowser = function(callback){
		var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>';
		var modalobj = util.dialog('请选择链接',[window.public_utility_link+'&callback=selectLinkComplete'],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
	...
	util.phoneBrowser = function(callback, page){
		var footer = '';
		var modalobj = util.dialog('一键拨号',[window.public_utility_phoneLink+'&callback=phoneLinkComplete&page='+ page],footer,{containerName:'link-container'});
		modalobj.modal({'keyboard': false});
	...
	util.showModuleLink = function(callback){
		var footer = '';
		var modalobj = util.dialog('模块链接选择',[window.public_utility_moduleLink+'&callback=moduleLinkComplete'], '');
		modalobj.modal({'keyboard': false});
		modalobj.find('.modal-body').css({'height':'700px','overflow-y':'auto' });
		modalobj.modal('show');
